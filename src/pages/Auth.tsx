...
const Auth = () => {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (session) {
        ...
        const {
          data: paymentMethod
        } = await supabase.from("payment_methods").select("*").eq("user_id", session.user.id).maybeSingle();
        if (paymentMethod) {
          navigate("/dashboard");
        } else {
  ...
  return <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex w-1/2 items-center justify-center p-12 relative" style={{
